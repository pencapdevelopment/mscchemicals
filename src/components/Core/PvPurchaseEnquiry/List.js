...
class List extends Component {
    ...
    loadObjects(offset, all, callBack) {
        if (!offset) offset = 1;
        var url = Const.server_url + Const.context_path + "api/" + this.props.baseUrl + "?projection=pv_purchase_list&page=" + (offset - 1);
                    ...
                    {this.state.objects.map((obj, i) => {
                        return (
                            <tr key={obj.id}>
                                <td>{i + 1}</td>
                                <td>
                                    <Link to={`/pvpurchase/${obj.id}`}>
                                        {obj.code}
                                    </Link>
                                </td>
                                <td>
                                    <Link to={`/prospective-vendor/${obj.company.id}`}>
                                        {obj.company.name}
                                    </Link>
                                </td>
                                <td>
