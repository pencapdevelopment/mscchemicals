...
class Negotiation extends Component {
    ...
    loadQuoteObj(id) {
        axios.get(Const.server_url + Const.context_path + "api/pvpurchase-quotation?enquiry.id=" + id + '&projection=pv_purchase_quotation_edit').then(res => {
    ...
    negotiationTraking(){
        axios.get( server_url + context_path + "api/pvpurchase-negotiation-tracking?reference.id="+this.props.currentId+"&sort=id,desc&projection=pv-purchase-negotiation-tracking").then(res => {
    ...
    loadPurchaseEnqObj(id) {
        axios.get(Const.server_url + Const.context_path + "api/" + this.props.baseUrl + "/" + id + '?projection=pv_purchase_edit').then(res => {
    ...
    saveNegotiation= (productsid) => {
        ...
        purchProdObj.reference = "/pvpurchase/"+this.state.purchaseEnqObj.id;
        purchProdObj.purchaseProduct = "/pvpurchase-products/"+purchProdObj.id;
        ...
        if(!(this.state.ngList1.ns1_readOnly && this.state.ngList1.ns2_readOnly && this.state.ngList1.ns3_readOnly)){
            axios.post( server_url + context_path + "api/pvpurchase-negotiation-tracking/", purchProdObj)
            .then(res => {
                axios.patch(server_url + context_path + "api/pvpurchase-products/"+purchProdObj.id,{id:purchProdObj.id,status:null});
    ...
    purchaseApprovalData(){
        ...
        ngTObj.pvpurchaseNegotiationTracking="/pvpurchase-negotiation-tracking/"+ngTObj.id;
        ngTObj.repository=this.props.baseUrl;
        ngTObj.reference = this.state.purchaseEnqObj.id;
        ngTObj.remark=ngTObj.remark;
        axios.post( server_url + context_path + "api/approvals/", {"pvpurchaseNegotiationTracking":ngTObj.pvpurchaseNegotiationTracking,"repository":ngTObj.repository,"reference":ngTObj.reference,"remark":ngTObj.remark})
        .then(res => {
    ...
    toggleModalNegotation = (productId) => {
        ...
        //     });
        axios.get(server_url + context_path + "api/pvpurchase-products/"+ productId+"?projection=pv_purchase-product")
        .then(res => {
           // var product = this.props.parentObj.products.find(p => p.id === res.data.id);
            this.setState({ purchProdObj: res.data, modalnegatation:!this.state.modalnegatation });
        });    
        axios.get( server_url + context_path + "api/pvpurchase-negotiation-tracking?purchaseProduct="+productId+"&page=0&size=1&sort=id,desc&projection=pv-purchase-negotiation-tracking")
        .then(res => {
    ...
    toggleRemarkNegotiation = (productId) => {
        ...
        axios.get( server_url + context_path + "api/pvpurchase-negotiation-tracking?pvpurchaseProduct="+productId+"&page=0&sort=id,desc&projection=pv-purchase-negotiation-tracking")
        .then(remarkResp => {
                                            ...
                                            <Table hover responsive>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr key={i}>
                                                        ...
                                                        <td>
                                                        <Button color='primary' size='small' disabled={product.status==='Approved'|| product.status===null} onClick={()=>this.toggleModalNegotation(product.id)} variant="contained">Negotiation</Button>
                                                        </td>
                                                        {/* <td>
