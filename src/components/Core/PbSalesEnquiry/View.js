...
class View extends Component {
    ...
    handleGenerateQuote(){
        swal({
            ...
            if(generate){
                ...
                axios.get(Const.server_url+Const.context_path+'api/prospective-buyer/'+this.state.obj.company.id).then(compRes => {
                        var newObj = {
                        code: Const.getUniqueCode('PSQ'),
                        company: "/prospective-buyer/"+compRes.data.id,
                        specification: '',
                        make: '',
                        packing: '',
                        gst: '',
                        ...
                        deliveryPeriod: '',
                        validity: '',
                        enquiry: '/pbsales/'+this.state.obj.id,
                    ...
                    axios.post(Const.server_url + Const.context_path + "api/pbsales-quotation", newObj).then(res =>{
                        this.setState({ loading: false });
                        this.toggleTab(1);
                    }).finally(() => {
                        this.setState({ loading: false });
    ...
    loadObj(id) {
        axios.get(Const.server_url + Const.context_path + "api/" + this.props.baseUrl + "/" + id + '?projection=pb_sales_edit')
        .then(res => {
    ...
    QuotationsCount(enqId) {
        axios.get(Const.server_url + Const.context_path + "api/pbsales-quotation?enquiry.id=" + enqId + '&projection=pb_sales_quotation_exists').then(res => {
            var list = res.data._embedded[Object.keys(res.data._embedded)[0]];
            this.setState({ isQuoteExists: list.length });
        });
    }
    loadAssignedUsers(id){
        axios.get(Const.server_url + Const.context_path + "api/" + this.props.baseUrl + "-user?projection=pb-sales-user&reference=" + id).then(res => {
            this.setState({
