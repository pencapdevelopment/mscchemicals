...
class Add extends Component {
    ...
    loadCompany(companyId) {
        axios.get(server_url + context_path + "api/prospective-buyer/" + companyId + '?projection=buyer_auto_suggest_product')
        .then(res => {
            ...
            axios.get(server_url + context_path + "api/buyer-contact?sort=id,asc&projection=buyer_contact_edit&page=0&size=1&buyer=" + companyId)
            .then(res => {
                if (res.data._embedded['buyer-contact'] && res.data._embedded['buyer-contact'].length) {
                    var formWizard = this.state.formWizard;
                    formWizard.obj.contactName = res.data._embedded['buyer-contact'][0].name;
    ...
    loadData() {
        axios.get(server_url + context_path + "api/" + this.props.baseUrl + "/" + this.state.formWizard.obj.id + '?projection=pb_sales_edit')
        .then(res => {
            var formWizard = this.state.formWizard;
    ...
    saveDetails() {
        ...
        if (!hasError) {
            ...
            selectedCompanies.forEach((comps,idx) => {
                ...
                newObj.company = '/prospective-buyer/' + comps.companyId;
                if(!this.state.formWizard.editFlag){
                    newObj.code = getUniqueCode('PSE');
                    ...
                    <div id="companyvalidatorDiv">
                        <div className="row">
                            <div className="col-3">
                                <fieldset>
                                    <FormControl>
                                        <AutoSuggest url="prospective-buyer"
                                            name="companyName"
                                            displayColumns="name"
                                            label="Company"
                                            onRef={ref => {
                                            ...
                                            projection="buyer_auto_suggest_product"
