...
class List extends Component {
    ...
    loadObjects(offset, all, callBack) {
        ...
        if(this.props.company) {
            url += "&company=" + this.props.company.id;
        }
        url = defaultDateFilter(this.state, url);
        if (all) {
            url += "&size=100000";
                ...
                <Table hover responsive>
                    <thead>
                        <Sorter columns={[
                            ...
                            { name: 'Email', sortable: false, param: 'email' },
                            { name: 'Phone', sortable: false, param: 'phone' },
                            { name: 'Company', sortable: true, param: 'company.name' },
                        ...
                        {this.state.objects.map((obj, i) => {
                            return (
                                <tr key={obj.id}>
                                    ...
                                    <td>
                                        {obj.type==='C' && <Link to={`/companies/${obj.company.id}`}>
                                                {obj.company.name}
                                            </Link>
                                        }
                                        {obj.type!=='C' &&    
